...
class FVP_Settings {
	...
	public function settings_init() {
		...
		add_settings_section( 'fvp-section',  __( 'Featured Videos', 'featured-video-plus'), array( $this, 'section' ), 'media' );
		...
		if( current_theme_supports( 'post-thumbnails' ) ) {
			add_settings_field( 'fvp-mode',      __( 'Display mode', 'featured-video-plus'), array( $this, 'mode' ),      'media', 'fvp-section' );
			add_settings_field( 'fvp-condition', __( 'Views', 'featured-video-plus'),        array( $this, 'condition' ), 'media', 'fvp-section' );
		}

		// general settings fields
		add_settings_field( 'fvp-sizing',   __( 'Video Sizing', 'featured-video-plus'),      array( $this, 'sizing' ),    'media', 'fvp-section' );
		add_settings_field( 'fvp-align',    __( 'Video Align', 'featured-video-plus'),       array( $this, 'alignment' ), 'media', 'fvp-section' );
		add_settings_field( 'fvp-defaults', __( 'Default Arguments', 'featured-video-plus'), array( $this, 'arguments' ), 'media', 'fvp-section' );
		add_settings_field( 'fvp-message',  __( 'Support', 'featured-video-plus'),           array( $this, 'message' ),   'media', 'fvp-section' );
	...
	public function section() {
		...
		if( ! current_theme_supports( 'post-thumbnails' ) ) {
			echo FVP_HTML::description(
				FVP_HTML::html( 'span', array( 'class' => 'bold' ), __( 'The current theme does not support featured images', 'featured-video-plus' ) ) .
	...
	public function sizing() {
		$options = get_option( 'fvp-settings' );
		$responsive = ! empty( $options['sizing']['responsive'] ) ? $options['sizing']['responsive'] : false;
		...
		echo FVP_HTML::conditional(
			FVP_HTML::labeled_input(
				...
				array(
					'type' => 'number',
					'size' => 4,
					'value' => ! empty( $options['sizing']['width'] ) ? $options['sizing']['width'] : 640,
				)
			),
			array(
	...
	public function message() {
		echo FVP_HTML::html(
			'p',
			sprintf(
				__('If you have found a bug or think a specific feature is missing, %slet me know%s in the support forum. Like this plugin? %sRate it%s or %sbuy me a cookie%s!', 'featured-video-plus'),
				'<a href="https://wordpress.org/support/plugin/featured-video-plus#plugin-title" title="Featured Video Plus Support Forum on WordPress.org" target="_blank" style="font-weight: bold;">', '</a>',
				'<a href="https://wordpress.org/support/view/plugin-reviews/featured-video-plus#plugin-title" title="Rate Featured Video Plus on WordPress.org" target="_blank" style="font-weight: bold;">', '</a>',
				'<a href="https://paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=AD8UKMQW2DMM6" title="Gift to the developer!" target="_blank" style="font-weight: bold;">', '</a>'
			)
		);
	}
	...
	public function help() {
		...
		$screen->add_help_tab( array(
			...
			'content' => implode( '', array(
				...
				FVP_HTML::html('p', sprintf( __( 'All parameters are optional. If %s the current post\'s id will be used. %s is either a string keyword (thumbnail, medium, large or full) or a 2-item array representing width and height in pixels, e.g. array(32,32).', 'featured-video-plus'), '<code>post_id == null</code>', '<code>$size</code>' ) ),
				FVP_HTML::html('p', sprintf( __( 'The functions are implemented corresponding to the original %sfunctions%s: They are intended to be used and to act the same way. Take a look into the WordPress Codex for further guidance:', 'featured-video-plus' ), '<a href="http://codex.wordpress.org/Post_Thumbnails#Function_Reference" target="_blank">' . __( 'Featured Image' ) . '&nbsp;', '</a>' ) ),
				FVP_HTML::unordered_list( array(
		...
		$screen->add_help_tab( array(
			...
			'content' => FVP_HTML::unordered_list( array(
				'<code>[featured-video-plus]</code><br />' .
					'<span>' . __( 'Displays the video in its default size.', 'featured-video-plus' ) . '</span>',
				'<code>[featured-video-plus width=560]</code><br />' .
					'<span>' . __( 'Displays the video with an width of 300 pixel. Height will be fitted to the aspect ratio.', 'featured-video-plus' ) . '</span>',
				'<code>[featured-video-plus width=560 height=315]</code><br />' .
					'<span>' . __( 'Displays the video with an fixed width and height.', 'featured-video-plus' ) . '</span>',
			))
		));
	}
